...
#' @param wordCount (Integer type.) The number of words in the text of the Article.
#' @param pagination (Text or Text or Text type.) Any description of pages that is not separated into pageStart and pageEnd; for example, "1-6, 9, 55" or "10-12, 46-49".
#' @param pageStart (Text or Integer or Text or Integer or Text or Integer type.) The page on which the work starts; for example "135" or "xiii".
#' @param pageEnd (Text or Integer or Text or Integer or Text or Integer type.) The page on which the work ends; for example "138" or "xvi".
#' @param articleSection (Text type.) Articles may belong to one or more 'sections' in a magazine or newspaper, such as Sports, Lifestyle, etc.
#' @param articleBody (Text type.) The actual body of the article.
...
#' @param printSection (Text type.) If this NewsArticle appears in print, this field indicates the print section in which the article appeared.
#' @param printPage (Text type.) If this NewsArticle appears in print, this field indicates the name of the page on which the article is found. Please note that this field is intended for the exact page name (e.g. A5, B18).
#' @param printEdition (Text type.) The edition of the print product in which the NewsArticle appears.
#' @param printColumn (Text type.) The number of the column in which the NewsArticle appears in the print edition.
#' @param dateline (Text type.) A [dateline](https://en.wikipedia.org/wiki/Dateline) is a brief piece of text included in news articles that describes where and when the story was written or filed though the date is often omitted. Sometimes only a placename is provided.Structured representations of dateline-related information can also be expressed more explicitly using [[locationCreated]] (which represents where a work was created e.g. where a news report was written).  For location depicted or described in the content, use [[contentLocation]].Dateline summaries are oriented more towards human readers than towards automated processing, and can vary substantially. Some examples: "BEIRUT, Lebanon, June 2.", "Paris, France", "December 19, 2017 11:43AM Reporting from Washington", "Beijing/Moscow", "QUEZON CITY, Philippines".
...
#'
#' @return a list object corresponding to a schema:NewsArticle
#'
#' @export

 NewsArticle <- function(id = NULL,
wordCount = NULL,
 pagination = NULL,
 pageStart = NULL,
 pageEnd = NULL,
