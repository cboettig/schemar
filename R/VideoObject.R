...
#' @param uploadDate (Date type.) Date when this media object was uploaded to this site.
#' @param requiresSubscription (Boolean type.) Indicates if use of the media require a subscription  (either paid or free). Allowed values are ```true``` or ```false``` (note that an earlier version had 'yes', 'no').
#' @param regionsAllowed (Place type.) The regions where the media is allowed. If not specified, then it's assumed to be allowed everywhere. Specify the countries in [ISO 3166 format](http://en.wikipedia.org/wiki/ISO_3166).
...
#' @param playerType (Text type.) Player type required&#x2014;for example, Flash or Silverlight.
...
#' @param encodesCreativeWork (CreativeWork type.) The CreativeWork encoded by this media object.
#' @param embedUrl (URL type.) A URL pointing to a player for a specific video. In general, this is the information in the ```src``` element of an ```embed``` tag and should not be the same as the content of the ```loc``` tag.
...
#' @param contentUrl (URL type.) Actual bytes of the media object, for example the image file or video file.
#' @param contentSize (Text type.) File size in (mega/kilo) bytes.
#' @param bitrate (Text type.) The bitrate of the media object.
#' @param associatedArticle (NewsArticle type.) A NewsArticle associated with the Media Object.
...
#' @param videoQuality (Text type.) The quality of the video.
#' @param videoFrameSize (Text type.) The frame size of the video.
#' @param transcript (Text or Text type.) If this MediaObject is an AudioObject or VideoObject, the transcript of that object.
#' @param thumbnail (ImageObject or ImageObject type.) Thumbnail image for an image or video.
...
#' @param caption (Text or Text type.) The caption for this object.
...
#'
#' @return a list object corresponding to a schema:VideoObject
#'
#' @export

 VideoObject <- function(id = NULL,
workExample = NULL,
 video = NULL,
 version = NULL,
