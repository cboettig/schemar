...
#' @param recordedAs (MusicRecording type.) An audio recording of the work.
#' @param musicalKey (Text type.) The key, mode, or scale this composition uses.
#' @param musicCompositionForm (Text type.) The type of composition (e.g. overture, sonata, symphony, etc.).
#' @param musicArrangement (MusicComposition type.) An arrangement derived from the composition.
#' @param lyrics (CreativeWork type.) The words in the song.
#' @param lyricist (Person type.) The person who wrote the words.
#' @param iswcCode (Text type.) The International Standard Musical Work Code for the composition.
#' @param includedComposition (MusicComposition type.) Smaller compositions included in this work (e.g. a movement in a symphony).
#' @param firstPerformance (Event type.) The date and place the work was first performed.
...
#'
#' @return a list object corresponding to a schema:MusicComposition
#'
#' @export

 MusicComposition <- function(id = NULL,
workExample = NULL,
 video = NULL,
 version = NULL,
