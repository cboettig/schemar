...
#' @param yield (Text or QuantitativeValue type.) The quantity that results by performing instructions. For example, a paper airplane, 10 personalized candles.
#' @param totalTime (Duration or Duration type.) The total time required to perform instructions or a direction (including time to prepare the supplies), in [ISO 8601 duration format](http://en.wikipedia.org/wiki/ISO_8601).
#' @param tool (Text or HowToTool or Text or HowToTool type.) A sub property of instrument. An object used (but not consumed) when performing instructions or a direction.
#' @param supply (Text or HowToSupply or Text or HowToSupply type.) A sub-property of instrument. A supply consumed when performing instructions or a direction.
#' @param steps (Text or ItemList or CreativeWork or Text or ItemList or CreativeWork type.) A single step item (as HowToStep, text, document, video, etc.) or a HowToSection (originally misnamed 'steps'; 'step' is preferred).
#' @param step (Text or HowToStep or HowToSection or CreativeWork type.) A single step item (as HowToStep, text, document, video, etc.) or a HowToSection.
#' @param prepTime (Duration or Duration type.) The length of time it takes to prepare the items to be used in instructions or a direction, in [ISO 8601 duration format](http://en.wikipedia.org/wiki/ISO_8601).
#' @param performTime (Duration or Duration type.) The length of time it takes to perform instructions or a direction (not including time to prepare the supplies), in [ISO 8601 duration format](http://en.wikipedia.org/wiki/ISO_8601).
#' @param estimatedCost (Text or MonetaryAmount or Text or MonetaryAmount type.) The estimated cost of the supply or supplies consumed when performing instructions.
#' @param suitableForDiet (RestrictedDiet or RestrictedDiet type.) Indicates a dietary restriction or guideline for which this recipe or menu item is suitable, e.g. diabetic, halal etc.
#' @param recipeYield (Text or QuantitativeValue type.) The quantity produced by the recipe (for example, number of people served, number of servings, etc).
#' @param recipeInstructions (Text or ItemList or CreativeWork type.) A step in making the recipe, in the form of a single item (document, video, etc.) or an ordered list with HowToStep and/or HowToSection items.
#' @param recipeIngredient (Text type.) A single ingredient used in the recipe, e.g. sugar, flour or garlic.
#' @param recipeCuisine (Text type.) The cuisine of the recipe (for example, French or Ethiopian).
#' @param recipeCategory (Text type.) The category of the recipe—for example, appetizer, entree, etc.
#' @param nutrition (NutritionInformation or NutritionInformation type.) Nutrition information about the recipe or menu item.
#' @param ingredients (Text type.) A single ingredient used in the recipe, e.g. sugar, flour or garlic.
#' @param cookingMethod (Text type.) The method of cooking, such as Frying, Steaming, ...
#' @param cookTime (Duration type.) The time it takes to actually cook the dish, in [ISO 8601 duration format](http://en.wikipedia.org/wiki/ISO_8601).
...
#'
#' @return a list object corresponding to a schema:Recipe
#'
#' @export

 Recipe <- function(id = NULL,
workExample = NULL,
 video = NULL,
 version = NULL,
