...
#' @param numTracks (Integer type.) The number of tracks in this album or playlist.
#' @param releaseOf (MusicAlbum type.) The album this is a release of.
#' @param recordLabel (Organization type.) The label that issued the release.
#' @param musicReleaseFormat (MusicReleaseFormatType type.) Format of this release (the type of recording media used, ie. compact disc, digital media, LP, etc.).
...
#' @param creditedTo (Person or Organization type.) The group the release is credited to if different than the byArtist. For example, Red and Blue is credited to "Stefani Germanotta Band", but by Lady Gaga.
#' @param catalogNumber (Text type.) The catalog number for the release.
...
#'
#' @return a list object corresponding to a schema:MusicRelease
#'
#' @export

 MusicRelease <- function(id = NULL,
workExample = NULL,
 video = NULL,
 version = NULL,
