...
#' @param isLiveBroadcast (Boolean type.) True is the broadcast is of a live event.
#' @param broadcastOfEvent (Event type.) The event being broadcast such as a sporting event or awards ceremony.
...
#' @param publishedOn (BroadcastService type.) A broadcast service associated with the publication event.
#' @param free (Boolean type.) A flag to signal that the item, event, or place is accessible for free.
...
#'
#' @return a list object corresponding to a schema:BroadcastEvent
#'
#' @export

 BroadcastEvent <- function(id = NULL,
videoFormat = NULL,
